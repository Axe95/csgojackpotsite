<?php    
	include "connect.php";
    if($_POST['steamid']=='33')
    $url=file_get_contents("http://steamcommunity.com/profiles/76561198235135036/inventory/json/730/2/?trading=1");
    else
	    ...
	    for($i=1;$i<=$total_items;$i++){
	        ...
	        echo "var total_price=0.00; 
				...
				$('#items-".$i."').click(function(){
				    ...
				    total_price=total_price + parseFloat(".$item_prices[$i-1].");
				    $('#items-".$i."').css('pointer-events', 'none');
				    $('#value-item-".$i."').css('color', '#525151');
				    document.getElementById('total-value').innerHTML = \"Total value: \"+total_price.toFixed(2)+\"$\";
