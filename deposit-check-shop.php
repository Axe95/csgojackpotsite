<?php
	...
	if ($steamids !== null && ($count_ids == count($market_name)
		...
		$query = "INSERT INTO send_table (partner_steam_id, bot_id, asset_id, it_name,
	 ...
	 if(isset($security))
     {
        $security = htmlspecialchars($security);
        $security = mysql_real_escape_string($security);
        $offerid = mysql_query("SELECT offer_id FROM send_table where partner_steam_id='".$steamid."' and offer_id<>0 and security_code='".$security."'  ORDER BY time_created DESC LIMIT 1");
